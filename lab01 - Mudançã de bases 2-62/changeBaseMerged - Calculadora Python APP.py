...
def char_para_valor(char):
    if char.isdigit():
        ...
        raise ValueError("Caractere inválido para conversão em valor: " + char)
...
def validar_numero(numero, base):
    ...
    for char in numero:
        if char not in caracteres_validos and char != '-':
            raise ValueError("Caractere inválido "+char+" para a base "+str(base)+".")
...
def juntar_partes(parte_inteira, parte_decimal):
    if parte_decimal:
        return str(parte_inteira)+"."+str(parte_decimal) #o f antes de " é para indicar que é uma string formatada( e tem o ponto entre as partes)
...
def main():
    ...
    try:
        ...
        print("O número "+ str(numero) +" na base "+ str(base_atual) +" é igual a "+ str(resultado) + " na base "+ str(base_nova))
