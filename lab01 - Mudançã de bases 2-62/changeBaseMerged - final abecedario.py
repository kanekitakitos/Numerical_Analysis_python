...
def converter_de_base_10_parte_decimal(numero, base):
    ...
    while numero > 0 and contador < DECIMAL_LIMIT: # criterio de paragem é a parte decimal de "numero" ser 0 ou o contador chegar a 8 (limite imposto pelo enunciado)
        ...
        numero = round(numero, DECIMAL_LIMIT)
        contador += 1
    return ''.join(digitos)
...
def main():
    ...
    try:
        ...
        for i in range(base_atual):
            abecedario += valor_para_char(i) + ' '
        print(f"Caracteres válidos para a base {base_atual} são: ")
        print(abecedario )
...
#se o ficheiro for corrido diretamente, chama a funçao main. se for importado como modulo, nao corre a funçao a nao ser que seja chamada
